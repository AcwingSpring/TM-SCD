...
class SelfAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
                 with_cffn=True, cffn_ratio=0.25, drop=0., drop_path=0.,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), with_cp=False,
                 use_dmsa_distance: bool = True):
        super().__init__()
        self.query_norm = norm_layer(dim)
        self.attn = DMSA(dim, num_heads, dropout=drop, use_distance=use_dmsa_distance)
        self.with_cffn = with_cffn
        self.with_cp = with_cp
        if with_cffn:
...
class DMSA(nn.Module):
    def __init__(self, embed_dims=256, num_heads=8, dropout=0.1, use_distance: bool = True):
        super().__init__()
...
class RadarBEVNet(nn.Module):
    def __init__(
        ...
        self.enable_injection = enable_injection and (enable_point and enable_transformer)
        self.enable_extraction = enable_extraction and (enable_point and enable_transformer)
    ...
    def forward(self, features, num_voxels, coors):
        ...
        for rfn in self.point_block:
            ...
            for bs in range(len(x_split)):
                ...
                # cross inter-branch interaction
        ...
        # merge branches according to enabled flags
        if self.enable_point and self.enable_transformer:
            c = self.out_linear(torch.cat([c, x], dim=-1))
        elif self.enable_transformer and not self.enable_point:
            # use transformer branch only
            pass  # keep c as is
        elif self.enable_point and not self.enable_transformer:
            # use point branch only
            c = x
        else:
            # should not happen; fallback to point features
            c = x
