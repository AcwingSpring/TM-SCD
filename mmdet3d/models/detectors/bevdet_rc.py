...
class BEVDet_RC(CenterPoint):
    ...
    def  (self, points, img, img_metas, radar, gt_bboxes_3d = None , **kwargs):  #add gt
        ...
        for i in range(0, len(img_feats)):
            ...
            if self.fusion_type == 'deformattn' and hasattr(self, 'DeformAttn1') and self.DeformAttn1 is not None:
                rad_i = self.radar_reduc_conv(rad_i)
                rad_tokens = rearrange(rad_i, 'b c h w -> b (h w) c')
                img_tokens = rearrange(img_i, 'b c h w -> b (h w) c')
                ...
                fusion_f1 = self.DeformAttn1(
                    query=self.with_pos_embed(rad_tokens, pos1),
                    reference_points=reference_point1,
                    input_flatten=self.with_pos_embed(img_tokens, pos2),
                ...
                fusion_f2 = self.DeformAttn2(
                    query=self.with_pos_embed(img_tokens, pos2),
                    reference_points=reference_point2,
                    input_flatten=self.with_pos_embed(rad_tokens, pos1),
                    input_spatial_shapes=torch.tensor([(self.bev_size, self.bev_size)]).to(device),
                    input_level_start_index=torch.tensor([0, self.bev_size * self.bev_size]).to(device),
                    input_padding_mask=None,
                )
                ...
                rad_reduced = self.radar_reduc_conv(rad_i)
                fusion_f = self.RadarConvFuser_fuse(rad_reduced, img_i)
            elif self.fusion_type == 'sum':
                rad_reduced = self.radar_reduc_conv(rad_i)
                fusion_f = rad_reduced + img_i
            else:
                # fallback
                rad_reduced = self.radar_reduc_conv(rad_i)
                fusion_f = self.RadarConvFuser_fuse(rad_reduced, img_i)
